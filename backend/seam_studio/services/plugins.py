...
# Plugins sit next to engines.json so a user manages both the same way: the
# repo root in a source checkout, the SEAM home (~/.seam/plugins) for a
# pip-installed run. Absent-safe — no directory means no plugins.
from ..core.paths import REPO_ROOT as _REPO_ROOT
from ..core.paths import SEAM_HOME as _SEAM_HOME

PLUGINS_DIR = (
    _REPO_ROOT / "plugins" if _REPO_ROOT is not None else _SEAM_HOME / "plugins"
)
...
# (plain objects) so a plugin needs no import from seam_studio.schemas.
