...
- **이동 액터 프레임별 재계산은 이미 연결됨.** `apply_actor_states()`(`sionna_backend.py:339`)가 시나리오 프레임마다 `SceneObject.position`·`.orientation` 설정; 솔버는 프레임당 1회 실행(`:776`, `:896`). → 효과 #1(시변 기하) 이미 생성.
- **확산 플래그 전달됨:** `diffuse_reflection=config.scattering`(`:781`).
- **재료:** `_apply_custom_materials`(`:973`) 는 `scattering_coefficient` 만 설정(`:1024-1027`). `xpd_coefficient` / `scattering_pattern` 은 미설정.
- **누락 물리(Gap):**
  - **(해소 — Design A)** `apply_actor_states` 가 이제 `velocities` 인자를 받아 액터별 `obj.velocity = mi.Vector3f(...)` 를 설정(`sionna_backend.py:417-424`) → 이동 액터가 경로별 Doppler 를 운반(더 이상 항등적 0 아님). 상세는 아래 「구현 완료」 절 참조.
  - `.cir(num_time_steps=…)` 미호출 → 프레임내 시간전개 없음; 경로는 곧장 `RayPath` 로 변환.
  - Lambertian/directive `alpha_r`·XPD 미노출.
- **시간 기반 존재:** `dt_s` 가 액터 궤적 스키마(`schemas/scene.py:155`)·시뮬 설정(`schemas/simulation.py:182`) 양쪽에 있고 `TrajectorySample.time_s = i*dt_s`(`services/trajectory.py:363/381/505`) — 프레임별 속도 `(pₙ₊₁ − pₙ)/dt_s` 직접 도출 가능.
...
**비용:** ~2–4일. 비용 동인: 확산 몬테카를로가 프레임당 solve 시간·메모리 증가(`samples_per_src`/`max_depth` 튜닝); 연접 시계열 출력 스키마·Doppler 확산 집계 설계; 확산 경로수가 `[:100]` 경로 캡(`trajectory.py:205`)을 넘지 않는지 검증. 가장 많은 물리, 가장 무거운 연산.
