...
> **현재 wiring 상태 (중요)**: 네 hook 모두 등록 자체는 성공하지만, 오늘 core
> 런타임에 실제로 연결되어 end-to-end로 효과가 나는 것은 `register_path_loss_model`
> 뿐이다 (`channel_analysis.py`가 `plugin_path_loss_models()`를 소비한다).
> `register_backend` / `register_ai_provider` / `register_exporter`로 등록한 것도
> getter(`plugin_backends()`/`plugin_ai_providers()`/`plugin_exporters()`)로 조회되고
> 등록은 정상 처리되지만, **아직 런타임 체인에 연결돼 있지 않다** —
> `get_backend`/`resolve_backend`는 정적 `_BACKENDS = {mock, sionna}`만 보고,
> provider 선택(`_select_provider`)과 export 경로도 plugin getter를 읽지 않는다.
> 이 세 hook을 실제로 소비하려면 core 통합 지점을 추가로 배선해야 한다.
...
[`docs/engines.md`](engines.ko.md) 참조 (버전별 차이는
[`docs/sionna_versions.md`](sionna_versions.ko.md)).
