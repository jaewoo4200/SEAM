...
- 결과는 프로젝트별로 보존됩니다: 경로 결과는 프로젝트 폴더의
  ...
  프로젝트를 열 때 함께 로드됩니다.
- Results 패널의 **A/B compare (pin A → re-solve → B)** 는 현재 경로 +
  채널 결과를 기준선 **A**로 고정합니다. 파라미터를 바꾸고 다시 solve하면
  경로 수, 최강 경로 파워·게인, 최초 도달, RSRP/RSRQ/RSSI/RSS/SINR,
  RMS 지연확산의 A / B / Δ 표와 출처 칩 두 개 — *solver config*(솔버 노브만)와
  *scene*(디바이스·재질·지오메트리; 의도한 파라미터 편집이면 *scene differs*로
  보이는 게 정상) — 가 표시됩니다. 핀은 클라이언트 전용이라 지우거나
  프로젝트를 바꾸기 전까지 유지되며, 디스크에는 아무것도 추가로 쓰지 않습니다.
