...
>
> **사전 요구사항(소스 경로):** **Python 3.11–3.14**와 **Node.js 20+**가
...
수동 설치·엔진 옵션·문제 해결은 **[INSTALL.md](INSTALL.ko.md)**, 첫 15분 실습은
**[TUTORIAL.md](TUTORIAL.ko.md)** 를 보세요.
...
- **SEAM-Agent (검색증강 로컬 AI 재질 저작)** — "이 건물은 한양대 FTC" 같은 힌트
  ...
  영역/박스→메시 역투영)과 결합해 wall/window/roof/frame 세그먼트별 재질 후보를 confidence·증거
...
- **UAV 액터 + 엔티티 뷰** — 차량/보행자/UAV/커스텀 산란체가 웨이포인트 궤적을
...
전체 데모 흐름은 [TUTORIAL.md](TUTORIAL.ko.md) 참조.
...
| [docs/ai_assistant.md](docs/ai_assistant.ko.md) | AI 제안 제공자, 규칙, provenance |
| [docs/engines.md](docs/engines.ko.md) | Sionna 엔진 버전 교체(별도 venv, `engines.json`) |
| [docs/sionna_versions.md](docs/sionna_versions.ko.md) | Sionna 버전별 기능·재질·모델 변천사(검증 문헌) |
| [docs/rtgui_parity.md](docs/rtgui_parity.ko.md) | NVlabs Sionna RT GUI 기능 패리티 매트릭스 |
| [docs/model_validation.md](docs/model_validation.ko.md) | 구현된 통신 모델·수식 전수 검증 문서 |
| [docs/dynamic_scattering.md](docs/dynamic_scattering.ko.md) | 동적 산란/도플러 조사·구현 노트 |
| [docs/ml_datasets.md](docs/ml_datasets.ko.md) | ML ground-truth 데이터셋 포맷·훈련 예제 |
...
연구에 SEAM Studio를 사용하셨다면 아래와 같이 인용해 주세요
(기계가독 메타데이터는 [CITATION.cff](CITATION.cff)에 있습니다):
